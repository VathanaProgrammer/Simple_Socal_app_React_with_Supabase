...
function CreateStory({ onClose}) {
    const [selectedImage, setSetlectedImage] = useState(null);
    
    const handleSelectImageChange = (e) => {
        const file = e.target.files[0];
        if(file){
            setSetlectedImage(URL.createObjectURL(file));
        }
    }
  ...
  return (
    <section className="bg-white border border-gray-300 w-[700px] h-[750px] flex flex-col rounded-[10px]">
      ...
      <header className="flex justify-between px-5 py-3 border-b border-gray-300">
        <h1 className="text-[22px] font-semibold text-[#3f3f3f]">Create Story</h1>
        <iconify-icon
      ...
      <div className="flex-1 flex flex-col p-4 gap-4 overflow-y-auto">
        ...
        {!selectedImage ? (
          ...
          >
            ...
            <input id="dropzone-file" onChange={handleSelectImageChange} type="file" className="hidden" />
          </label>
        </div>
        ) : (
            <div className="flex justify-center">
                <img src={selectedImage} alt={selectedImage} className="w-full max-h-64 object-cover rounded-lg" />
            </div>
        )}
      </div>
